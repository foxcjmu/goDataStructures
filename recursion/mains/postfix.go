// postfix.go: Ask users for a postfix expression, evaluate it using a recursive evaluation
...
func main() {
	fmt.Printf("Postfix expression evaluator.\n")

	console := bufio.NewReader(os.Stdin) // where we read from

	for {
		var (
			exp []byte // postfix expression typed by the user
		...
		if v, err := recursion.EvalPostfixRecursive(string(exp)); err != nil {
			fmt.Printf("%v\n", err)
		} else {
			fmt.Printf("%d\n", v)
		}
		if v, err := recursion.EvalPostfixStack(string(exp)); err != nil {
			fmt.Printf("%v\n", err)
		} else {
			fmt.Printf("%d\n", v)
		}
