// prefix.go: Ask users for a prefix expression, evaluate it using a recursive evaluation
...
func main() {
	fmt.Printf("Prefix expression evaluator.\n")

	console := bufio.NewReader(os.Stdin) // where we read from

	for {
		var (
			exp []byte // prefix expression typed by the user
		...
		if v, err := recursion.EvalPrefixRecursive(string(exp)); err != nil {
			fmt.Printf("%v\n", err)
		} else {
			fmt.Printf("%d\n", v)
		}
		if v, err := recursion.EvalPrefixStack(string(exp)); err != nil {
			fmt.Printf("%v\n", err)
		} else {
			fmt.Printf("%d\n", v)
		}
